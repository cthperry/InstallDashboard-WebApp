    ...
    <main className="min-h-svh flex items-center justify-center p-4">
      <Card className="w-full max-w-md py-0">
        <CardHeader className="border-b pb-4">
          <CardTitle className="text-xl font-bold">裝機狀態 Dashboard</CardTitle>
          <div className="text-xs text-muted-foreground">Premtek 內部系統 · v{appVersion}</div>
        </CardHeader>
        <CardContent className="space-y-4 pt-5">
          {user ? (
            <div className="space-y-2">
              <div className="text-sm text-muted-foreground">已登入：{user.email}</div>
          ...
          <div className="rounded-md border border-border bg-muted/40 px-3 py-2 text-xs leading-5 text-muted-foreground">
            僅限 <span className="font-semibold text-foreground">@premtek.com.tw</span> 帳號；資料權限以 Firestore Rules 為準。
          </div>
        </CardContent>
      </Card>
    </main>
