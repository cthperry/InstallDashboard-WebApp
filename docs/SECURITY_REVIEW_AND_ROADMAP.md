...
- Next.js was updated to `16.2.9`; top-level PostCSS was updated to `8.5.14`.
...
- Firestore writes still allow all company engineers to create/update installation and equipment records. If this data is sensitive by region or team, keep the two role types (`admin`, `engineer`) and add scoped permissions such as region/customer scope fields instead of introducing new role names.
...
1. Role model v2: keep exactly two role types, `admin` and `engineer`; add optional permission scope fields only if region/customer boundaries are needed.
